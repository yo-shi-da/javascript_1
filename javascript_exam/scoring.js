...
$(document).ready(function () {
  ...
  // 「国語、英語、数学、理科、社会」の点数（入力値）を取得して合計点と平均点を出すロジック
    function score_indicate() {
      ...
      let subject_points = [
        Number($('#national_language').val()), // numberは文字列を数値に変換。
        Number($('#english').val()),
    ...
    // 最終的なジャッジのロジックを作ります。
    function judgement() {
      // 変数「achievement」に「get_achievement()の戻り値」を代入します。関数：平均点数から「A、B、C、D」にランク分け。
      let achievement = get_achievement();
      // 変数「pass_or_failure」に「get_pass_or_failure()の戻り値」を代入します。関数：各教科の点数を取得し、取得した点数から「合格、不合格」の判断を下す。
    ...
    $('#btn-declaration').click(function () {
      $("#declaration").remove();      
      $(".final").append('<label id="declaration"></label>');
